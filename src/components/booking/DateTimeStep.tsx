...
import { Scissors, User, Clock, CalendarDays, ChevronLeft, ChevronRight, Info } from "lucide-react";
...
export function DateTimeStep({
    ...
    return (
      <div className="mb-5 last:mb-0">
        <div className="flex items-center gap-2 mb-2.5">
          ...
          <span className="text-xs text-muted-foreground/60 ml-auto">{slots.length} {slots.length === 1 ? "horario" : "horarios"}</span>
        </div>
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
      <h2 className="text-xl sm:text-2xl font-bold mb-1">Escolha data e horario</h2>
        ...
        <div className="animate-in fade-in duration-200">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
              ...
              {timeSlots.length} {timeSlots.length === 1 ? "horario" : "horarios"}
            </span>
          </div>

          {/* Show resolved professional for "any" mode */}
          {resolvedProfessional && selectedTime && (
            <div className="rounded-xl bg-primary/5 border border-primary/15 p-3 flex items-center gap-3 mb-4 animate-in fade-in duration-200">
              <Info className="h-4 w-4 text-primary shrink-0" />
              <p className="text-xs text-foreground">
                Profissional disponivel: <span className="font-bold">{resolvedProfessional.name}</span>
              </p>
            </div>
          )}
          ...
          {timeSlots.length === 0 ? (
            <div className="text-center py-10 text-muted-foreground rounded-2xl border border-dashed border-border">
              ...
              <p className="text-sm font-medium">Nenhum horario disponivel</p>
