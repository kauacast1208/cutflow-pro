...
interface PlanDef {
  ...
  features: string[];
  cta: string;
  ctaVariant: "outline" | "hero" | "default";
  popular?: boolean;
...
const faqs = [
  ...
  },
  {
    ...
    a: "Você paga 12 meses com desconto de até 20%. O valor é cobrado de uma vez ou parcelado.",
  },
];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-10 sm:mb-14">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-3 rounded-full border border-border/60 bg-card/80 backdrop-blur-sm p-1.5"
          >
            <button
        ...
        {/* Plans grid — 3 main + 2 wide below */}
        ...
        <motion.div
          ...
          className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-4 sm:gap-6 mt-10 sm:mt-12 text-xs sm:text-[13px] text-muted-foreground"
        >
...
function PlanCard({
  ...
  const cardContent = (
    <motion.div
      ...
      className={`rounded-2xl border flex flex-col relative transition-all duration-300 group ${
        isPopular
    ...
    >
      ...
      <div className={`relative ${wide ? "md:flex-1" : ""}`}>
        ...
        <div className="mb-4 sm:mb-5">
          <div className="flex items-center gap-2.5 mb-2">
            ...
            <h3
              className={`font-bold ${
                isPopular ? "text-xl sm:text-2xl" : "text-lg sm:text-xl"
              }`}
            >
              {plan.label}
            </h3>
      ...
      <div className={`relative ${wide ? "md:flex-1" : ""} flex flex-col flex-1`}>
        {/* Features */}
        <ul
          className={`space-y-2.5 mb-6 sm:mb-7 flex-1 ${
            wide ? "sm:grid sm:grid-cols-2 sm:gap-x-6 sm:gap-y-2.5 sm:space-y-0" : ""
          }`}
        >
          {plan.features.map((f) => (
        ...
        {plan.externalLink ? (
          ...
          >
            <Button
              variant={plan.ctaVariant === "hero" ? "hero" : "outline"}
              className={`w-full rounded-xl gap-2 ${
                isPopular ? "h-12 sm:h-13 text-[15px]" : "h-11 sm:h-12 text-sm"
              }`}
            >
