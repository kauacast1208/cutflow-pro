...
serve(async (req) => {
  ...
  try {
    ...
    // Never expose raw Stripe/internal errors to client
    return new Response(JSON.stringify({ error: "Não foi possível abrir o portal de pagamento. Tente novamente." }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 500,
    });
  }
